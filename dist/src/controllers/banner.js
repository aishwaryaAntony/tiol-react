...
exports.fetch_all_banners = async (req, res, next) => {
    ...
    try {
        let fetchAllBanners = await _models2.default.Banner.findAll({});
        res.status(200).json({
            status: 'success',
...
exports.fetch_banner_by_id = async (req, res, next) => {
    ...
    try {
        let { id } = req.params;
        let fetchBanner = await _models2.default.Banner.findOne({
            where: {
                id: id
            }
        });
...
exports.create_new_banner = async (req, res, next) => {
    ...
    try {
        ...
        if (req.file !== undefined) {
            let newBanner = await _models2.default.Banner.create({
                banner_name,
                user_profile_id,
                status
            });

            await (0, _attachmentServices.addAttachment)(req.file, _constants.BANNERS_BUCKET_NAME, newBanner.id, null);
            newBanner = await _models2.default.Banner.findOne({ where: { id: newBanner.id } });
...
exports.update_banner = async (req, res, next) => {
    ...
    try {
        ...
        if (fetchBanner === null) {
            ...
            let updatedBanner = await _models2.default.Banner.update({
            ...
            if (fetchUpdatedBanner !== null) {
                if (req.file !== undefined) {
                    if (fetchUpdatedBanner.mongo_id !== null) {
                        await (0, _attachmentServices.deleteAttachment)(fetchUpdatedBanner.mongo_id, _constants.BANNERS_BUCKET_NAME);
                    }
                    await (0, _attachmentServices.addAttachment)(req.file, _constants.BANNERS_BUCKET_NAME, fetchUpdatedBanner.id, null);
                }
                fetchUpdatedBanner = await _models2.default.Banner.findOne({ where: { id: fetchUpdatedBanner.id } });
                return res.status(200).json({
                    status: 'success',
